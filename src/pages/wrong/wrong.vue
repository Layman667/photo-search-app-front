<template>
  <view class="wrong-container">
    ...
    <view v-else-if="list.length > 0" class="list-section">
      <view class="list-item" v-for="item in list" :key="item.wrongId" @click="viewDetail(item)">
        ...
        <view class="delete-btn" @click.stop="handleDelete(item.wrongId)">
...
<script>
import { getWrongList, deleteWrong, clearWrong } from '@/api/topic'
...
export default {
  ...
  methods: {
    async loadList() {
      this.loading = true
      try {
        const res = await getWrongList(this.page, this.pageSize)
