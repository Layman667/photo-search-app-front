<template>
  <view class="history-container">
    ...
    <view v-else-if="list.length > 0" class="list-section">
      <view class="list-item" v-for="item in list" :key="item.historyId" @click="viewDetail(item)">
        ...
        <view class="delete-btn" @click.stop="handleDelete(item.historyId)">
    ...
    <view v-else class="empty-section">
      ...
      <text class="empty-text">暂无搜题历史</text>
    </view>
  </view>
</template>

<script>
import { getHistoryList, deleteHistory, clearHistory } from '@/api/topic'
...
export default {
  ...
  methods: {
    async loadList() {
      this.loading = true
      try {
        const res = await getHistoryList(this.page, this.pageSize)
        this.list = res.data.list || []
      } catch (err) {
        console.error('加载搜题历史失败:', err)
      } finally {
        this.loading = false
      }
    },
    async handleDelete(historyId) {
      uni.showModal({
        title: '提示',
        content: '确定删除这条记录吗？',
        success: async (res) => {
